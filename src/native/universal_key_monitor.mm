...
// static const CFTimeInterval DOUBLE_TAP_THRESHOLD = 0.5; // Unused - commented out
...
static void hidInputValueCallback(void *context, IOReturn result, void *sender, IOHIDValueRef value) {
    ...
    // Function key is on Generic Desktop usage page (0x01) with usage 0x18 (or similar)
    // We need to intercept and suppress it
    if (usagePage == kHIDPage_GenericDesktop && usage == 0x18 && monitoredKey == "fn") {
        // Handle our callback
...
CGEventRef eventTapCallback(CGEventTapProxy proxy, CGEventType type, CGEventRef event, void *refcon) {
    ...
    if (monitoredKey == "fn+ctrl") {
        ...
        if (type == kCGEventFlagsChanged) {
            ...
            bool hasOtherModifiers = (flags & (kCGEventFlagMaskCommand | kCGEventFlagMaskShift | kCGEventFlagMaskControl | kCGEventFlagMaskAlternate)) != 0;
        ...
        if (type == kCGEventKeyDown || type == kCGEventKeyUp) {
            ...
            if (keyCode == 63 || keyCode == 179 || keyCode == 0x3F) {  // Common fn key codes
...
Napi::Value StartMonitoring(const Napi::CallbackInfo& info) {
    ...
    if (info.Length() < 2 || !info[0].IsString() || !info[1].IsFunction()) {
        Napi::TypeError::New(env, "Usage: startMonitoring(keyName, callback)").ThrowAsJavaScriptException();
        return env.Null();
    }
    ...
    if (keyFlags.find(keyName) == keyFlags.end()) {
        Napi::TypeError::New(env, "Unsupported key. Supported keys: fn, option, control, command").ThrowAsJavaScriptException();
        return env.Null();
    }
    ...
    if (keyName == "fn" || keyName == "fn+ctrl") {
        ...
        if (eventTap) {
            ...
            NSLog(@"AGGRESSIVE function key monitoring enabled - will SUPPRESS all fn events to prevent emoji picker");
        } else {
    ...
    if (keyName != "fn" && keyName != "fn+ctrl") {
        ...
        globalMonitor = [NSEvent addGlobalMonitorForEventsMatchingMask:(NSEventMaskFlagsChanged | NSEventMaskKeyDown | NSEventMaskKeyUp)
            ...
            if ([event type] == NSEventTypeFlagsChanged) {
                ...
                bool currentKeyState = (flags & targetFlag) != 0;
        ...
        localMonitor = [NSEvent addLocalMonitorForEventsMatchingMask:(NSEventMaskFlagsChanged | NSEventMaskKeyDown | NSEventMaskKeyUp)
            ...
            return event; // Allow other events for non-fn keys
        }];
    }
...
Napi::Object Init(Napi::Env env, Napi::Object exports) {
    ...
    exports.Set(Napi::String::New(env, "getSupportedKeys"), Napi::Function::New(env, GetSupportedKeys));
    return exports;
}
